...
<#
.SYNOPSIS
    Tech Research Skill Installer for Windows.

.DESCRIPTION
    Installs the tech-research skill to system or project agent skill directories.
...
function Test-PythonDeps {
    ...
    try {
        ...
        $missingDeps += "duckduckgo-search"
    }
    
    try {
        python -c "import requests" 2>$null
    } catch {
        $missingDeps += "requests"
    }
    
    try {
        python -c "import bs4" 2>$null
    } catch {
        $missingDeps += "beautifulsoup4"
    }
    ...
    if ($missingDeps.Count -gt 0) {
        ...
        if ($response -match "^[Yy]$") {
            ...
            Write-Warning "Skipping dependency installation. Skill may not work properly."
        }
    } else {
...
function Main {
    ...
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  python $SkillRoot\scripts\research.py `"你的技术问题`""
}
